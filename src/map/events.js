...
import { equipTechnique } from '../battleTechniques.js';
import {
  EQUIP_TAGPOOL_PO_A,
  EQUIP_TAGPOOL_PO_B,
} from '../equipment/tags.js';

function pickEquipmentIdForEvent(game) {
  const equipped = getEquippedEquipmentIds(game);
  if (equipped.length <= 0) return null;
  const labels = equipped
    .map((id, idx) => `${idx + 1}. ${getEquipmentDef(id)?.name || id}`)
    .join('\n');
  const raw = window.prompt(`选择一件装备：\n${labels}\n输入序号（取消则放弃）`, '1');
  const n = Number(raw);
  if (!Number.isFinite(n)) return null;
  const idx = Math.max(1, Math.min(equipped.length, Math.floor(n))) - 1;
  return equipped[idx] || null;
}
...
export function resolveEventChoice(game, room, choiceKey, { addLog } = {}) {
  ...
  if (eventId === 'event_3') {
    if (choiceKey === 'A') {
      const equipmentId = pickEquipmentIdForEvent(game);
      if (!equipmentId) return { text: '你看了看背包，没有进行任何改动。', leave: true };
      rerollEquipmentTag(game, equipmentId);
      return { text: '你对装备进行了重随词条。', leave: true };
    }
    if (choiceKey === 'B') {
      const techId = grantMapTechnique(game);
      if (techId) {
        addLog?.('log-ab', `📘 事件奖励：获得战技 ${techId}（已自动装备到对应攻击槽）。`);
      }
