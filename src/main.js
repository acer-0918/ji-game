...
import { POWER_RELIC_DEFS, SHOP_ITEMS, getAbilityDefsForClass, getPowerRelicDef } from './data.js';
import {
  calcDamage,
  describeAttack,
  formatSingleAction,
...
function startBattle(node, keepSnapshot=false) {
  ...
  G.battle = {round:1, phase:'select', pAction:null, eAction:null, lastPlayerAction:null, lastEnemyAction:null, popcornPending:false, roundDisabledActions:[], reunionDamageBonus:0, killedByDestinedFirstSight:false};
...
function mainSelect(category) {
  ...
  if (category === 'sp') {
    if (G.player.classKey !== 'mage') return;
